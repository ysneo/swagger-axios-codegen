...
function createDefinitionClass(className, properties, required) {
    ...
    for (const [k, v] of propertiesEntities) {
        ...
        let { propType, isEnum, isArray, isType, ref, isUnionType, isCombinedType } = propTrueType_1.propTrueType(v);
        if (isEnum) {
        ...
        if (isUnionType) {
            let typeName = `All${pascalcase_1.default(k)}Types`;
        ...
        if (isCombinedType) {
            let typeName = `Combined${pascalcase_1.default(k)}Types`;
            let types = propType.split(',');
            enums.push({
                name: typeName,
                text: `export type ${typeName} = ${types.join(' & ')};`
